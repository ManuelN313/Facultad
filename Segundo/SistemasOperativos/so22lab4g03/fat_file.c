...
// this function doesn't write on the nentry but on the child->pos_in_parent entry
static void write_dir_entry(fat_file parent, fat_file file) {
...
void fat_file_unlink(fat_file file, fat_file parent) {
    /*
    - Primer idea:
    - marcamos como libres los clusters del file que queremos "remover" (rm file)
    - guardamos los cambios a disco
    */
    u32 current_cluster = 0, next_cluster = 0;
