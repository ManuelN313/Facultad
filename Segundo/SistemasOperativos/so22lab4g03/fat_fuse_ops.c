...
static void fat_fuse_log_activity(char *operation_type, fat_file file) {
    ...
    if (fat_file_cmp_path(file, strdup(BB_LOG_FILE)) != 0) {
...
static void fat_fuse_read_children(fat_tree_node dir_node) {
    ...
    fat_tree_node bb_dir_node = NULL;
    ...
    for (GList *l = children_list; l != NULL; l = l->next) {
        vol->file_tree = fat_tree_insert(vol->file_tree, dir_node, (fat_file)l->data);
    }

    if (fat_file_cmp_path(dir, strdup("/")) == 0) {
      bb_init_log_dir();
      DEBUG("Initialized bb_dir from %s\n", dir->filepath);
      // when the image is mounted, fat_fuse_read_children is only called for
      // root, to create the log file right after mounting we call it again for
      // BB_DIRNAME
      bb_dir_node = fat_tree_node_search(vol->file_tree, BB_DIRNAME);
      fat_fuse_read_children(bb_dir_node);
    }

    if (fat_file_cmp_path(dir, strdup(BB_DIRNAME)) == 0) {
      // log file should be located on the first entry of the bb_dir, if it's
      // not, we create it
      if (g_list_nth_data(children_list, 0) == NULL) {
        fat_fuse_mknod(BB_LOG_FILE, 0, 0);
        DEBUG("Created new fs.log in %s\n", dir->filepath);
      }
    }
}
...
int fat_fuse_readdir(const char *path, void *buf, fuse_fill_dir_t filler,
    ...
    child = children;
    while (*child != NULL) {
        if (fat_file_cmp_path(*child, strdup(BB_DIRNAME)) != 0) {
...
// deletes the directory specified in @path
// FIXME: it seems entries are duplicated thus making it necessary to call rmdir twice to
// permanently remove a directory
int fat_fuse_rmdir(const char *path) {
    errno = 0;
    fat_volume vol = NULL;
    fat_file dir = NULL;
    fat_file parent = NULL;
    fat_tree_node dir_node = NULL;
	fat_file *elem = NULL;
    ...
    if (!fat_file_is_directory(dir)) {
        fat_error("Error! Not a directory\n");
        errno = ENOTDIR;
        return -errno;
	}
	
	elem = fat_tree_flatten_h_children(dir_node);
	if (elem[0] != NULL) {
		free(elem);
		elem = NULL;
...
int fat_fuse_unlink(const char * path) {
    /*
        ...
        - fijarnos si existe el file
        - si existe, lo obtenemos y lo guardamos en una variable fat_file
        - nos ayudamos de la funcion fat_file_unlink para marcar sus clusters como libres -> done
        - por ultimo, tenemos que eliminarlo del directory tree -> done
    */
    errno = 0;
    fat_volume vol = get_fat_volume();
    fat_file file = NULL, parent = NULL;
