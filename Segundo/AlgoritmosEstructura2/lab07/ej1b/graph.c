...
static bool invrep(graph_t g) {
    ...
    bool result = g->costs != NULL && g->max_vertexs != 0;
    unsigned int i = 0u;
    while (result && i < g->max_vertexs) {
        result = result && g->costs[i] != NULL;
        ++i;
    } 
    return g != NULL && result;
}
...
graph_t graph_empty(unsigned int max_vertexs)
{
    ...
    graph->costs = malloc(max_vertexs * sizeof(cost_t *));
    for (unsigned int i = 0; i < graph->max_vertexs; i++) {
        graph->costs[i] = malloc(max_vertexs * sizeof(cost_t));
    }
    for (unsigned int i = 0; i < graph->max_vertexs; ++i) {
