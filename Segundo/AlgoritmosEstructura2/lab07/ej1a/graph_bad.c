...
struct graph_data {
    cost_t *costs;            //cost_t array for the edges
    unsigned int max_vertexs; //unsigned int number of vertexs
};

static unsigned int g_index(vertex_t i, vertex_t j, unsigned int max_vertexs) { //formula to calculate indextation for the cost_t array
...
graph_t graph_empty(unsigned int max_vertexs)
{
    ...
    cost_t costs_array[max_vertexs * max_vertexs];  //No me gusta que este usando memoria stack

    for (unsigned int i = 0; i < max_vertexs * max_vertexs; ++i) { 
      costs_array[i] = cost_inf(); //inicializa el arregla max_vertexs * max_vertexs con todos los pesos de los vertices en inf
    }
    // Save matrix in the structure
    graph->costs = costs_array; 
    // Hago que el puntero cos_t costs apunte al arreglo de memoria estatica, el cual va a ser liberada una vez termine esta funcion 
    // y por lo tanto el puntero va a estar apuntando a basura
