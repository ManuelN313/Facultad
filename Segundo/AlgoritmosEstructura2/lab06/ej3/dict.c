...
static bool is_abb(dict_t d, key_t min, key_t max){
    bool b = true;
    if (d != NULL){
        if(key_less(d->key, min) || key_less(max, d->key)){
            ...
            b = is_abb(d->left, min, d->key) && is_abb(d->right, d->key, max);
        }
    }
    return b;
}

static key_t getMin(dict_t d){
    key_t min;
    dict_t p = d;
    while(p->left != NULL){
        p = p->left;
