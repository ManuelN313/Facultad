import numpy as np
from random import random

# En esta version los profes no reescriben las funciones.
# Puede que sea mas util esta version para el parcial ya
# que no tengo que reescribir la funcion, solamente debo fijarme
# en que caso estoy y usar la version de MonteCarlo correspondiente

# Integral Monte Carlo en el intervalo (0,1)
def MonteCarlo_01(fun, Nsim):
    Integral = 0
    for _ in range(Nsim):
        Integral += fun(random())
    return Integral/Nsim

# Integral Monte Carlo en el intervalo (a,b)
def MonteCarlo_ab(fun, a, b, Nsim):
    Integral = 0
    for _ in range(Nsim):
        Integral += fun(a + (b-a)*random())
    return Integral*(b-a)/Nsim #Aquí multiplico una sola vez por (b-a)

# Integral Monte Carlo en el intervalo (0,inf)
def MonteCarlo_inf(fun, Nsim):
    Integral=0
    for _ in range(Nsim):
        u=random()
        Integral+= fun(1/u-1)/(u**2)
    return Integral/Nsim

N = [100,1000,10000,100000,1000000]

print("Ejercicio A")

def g_a(u):
    return (1 - u**2)**(3/2)

for i in range(len(N)):
    print('Integral para Nsim =',N[i])
    print(MonteCarlo_01(g_a,N[i]))

print("Ejercicio B")

def g_b(u):
    return (u/(u**2 - 1))

for i in range(len(N)):
    print('Integral para Nsim =',N[i])
    print(MonteCarlo_ab(g_b,2,3,N[i]))

print("Ejercicio C")

def g_c(u):
    return u/((u**2 + 1)**(2))
     
for i in range(len(N)):
    print('Integral para Nsim =',N[i])
    print(MonteCarlo_inf(g_c,N[i])) 

print("Ejercicio D")

def g_d(u):
    return 2*np.exp(-u**2) # Multiplico por 2 pues la funcion np.exp(-u**2) es par
                           # y por lo tanto la integral de - inf a + inf se puede 
                           # reescribir como 2 veces la integral de 0 a + inf
     
for i in range(len(N)):
    print('Integral para Nsim =',N[i])
    print(MonteCarlo_inf(g_d,N[i]))

# Integral Monte Carlo en el intervalo (0,1)x(0,1)
def MonteCarlo_01_2(fun, Nsim):
    Integral = 0
    for _ in range(Nsim):
        Integral += fun(random(), random())
    return Integral/Nsim

# Integral Monte Carlo en el intervalo (a,b)x(c,d)
def MonteCarlo_ab_2(fun,a,b,c,d, Nsim):
    Integral = 0
    for _ in range(Nsim):
        Integral += fun(a + (b-a)*random(), c + (d-c)*random())
    return Integral*(b-a)*(d-c)/Nsim

# Integral Monte Carlo en el intervalo (0,inf)x(0,inf)
def MonteCarlo_inf_2(g, Nsim):
    Integral=0
    for _ in range(Nsim):
        u1=random()
        u2=random()
        Integral+= g(1/u1-1, 1/u2-1)/((u1**2)*(u2**2))
    return Integral/Nsim

print("Ejercicio E")

def g_e(u,v):
    return np.exp((u + v)**2)

for i in range(len(N)):
    print('Integral para Nsim =',N[i])
    print(MonteCarlo_01_2(g_e,N[i]))

print("Ejercicio F")

def I(x, y): # Indicadora
    if y < x:
        return 1
    else:
        return 0

def g_f(x, y):
    return np.exp(-(x+y))*I(x, y)
     
for i in range(len(N)):
    print('Integral para Nsim =',N[i])
    print(MonteCarlo_inf_2(g_f,N[i]))
    